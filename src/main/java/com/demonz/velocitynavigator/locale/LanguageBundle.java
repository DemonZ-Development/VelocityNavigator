...
public final class LanguageBundle {
    ...
    private static LanguageBundle createDefaults() {
        ...
        strings.put("auth.login_prompt", "<yellow>Please authenticate with /login <password>.</yellow>");
        strings.put("auth.register_prompt", "<yellow>Please register with /register <password> [repeat].</yellow>");
        strings.put("auth.login_success", "<green>Login successful.</green>");
        strings.put("auth.register_success", "<green>Registration complete. You are now authenticated.</green>");
        strings.put("auth.already_registered", "<yellow>You are already registered. Use /login.</yellow>");
        strings.put("auth.not_registered", "<yellow>You are not registered. Use /register first.</yellow>");
        strings.put("auth.password_mismatch", "<red>The repeated password does not match.</red>");
        strings.put("auth.invalid_registration_password", "<red>Registration failed. Use a password between <min> and 128 characters.</red>");
        strings.put("auth.invalid_credentials", "<red>Invalid password.</red>");
        strings.put("auth.too_many_attempts", "<red>Too many failed attempts. Please wait <time> second(s) before trying again.</red>");
        strings.put("auth.disabled", "<red>Authentication is not enabled.</red>");
        strings.put("auth.register_usage", "<aqua>Usage: /register <password> [repeat]</aqua>");
        strings.put("auth.login_usage", "<aqua>Usage: /login <password></aqua>");
        strings.put("auth.logout_success", "<yellow>You have been logged out.</yellow>");
        strings.put("auth.form.register_title", "Create your account");
        strings.put("auth.form.login_title", "Log in");
        strings.put("auth.form.password_visible", "Password (visible while typing - use privacy)");
        strings.put("auth.form.repeat_password", "Repeat password");
        strings.put("auth.form.password_placeholder", "Enter your password");
        strings.put("auth.form.command_fallback", "<yellow>Authentication form closed. Use /login or /register in chat.</yellow>");
        strings.put("auth.form.try_again", "<red>Authentication failed. Please try again.</red>");
