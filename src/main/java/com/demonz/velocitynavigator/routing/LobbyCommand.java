...
import com.demonz.velocitynavigator.menu.BedrockFormService;
import com.demonz.velocitynavigator.menu.JavaInventoryMenuService;
import com.demonz.velocitynavigator.menu.JavaMenuService;
...
public final class LobbyCommand implements SimpleCommand {
    ...
    private final AtomicLong degradedCounter = new AtomicLong(0);
    ...
    public void execute(Invocation invocation) {
        ...
        if (plugin.isAuthenticationPending(player)) {
            player.sendMessage(MessageFormatter.render(
                    "<yellow>Please authenticate before using lobby routing.</yellow>", player));
    ...
    private void connectFromMenuSelection(Player player, Config config, String targetServer) {
        plugin.previewRoute(player)
                .thenAccept(decision -> ConnectionWorkflow.connectFromSelection(
                        plugin, player, config, decision, targetServer, "chat_menu"))
                .exceptionally(throwable -> {
                    plugin.cooldowns().clear(player.getUniqueId());
    ...
    private String pickDegraded(List<String> candidates, Config.SelectionMode mode) {
        ...
        return switch (mode) {
            ...
            case POWER_OF_TWO, LEAST_CONNECTIONS, WEIGHTED_ROUND_ROBIN, CONSISTENT_HASH, LATENCY, GEO_DISTANCE ->
                candidates.get(java.util.concurrent.ThreadLocalRandom.current().nextInt(candidates.size()));
        };
    }

    public static String pickLeastPlayers(List<String> candidates, ToIntFunction<String> playerCount) {
