...
public record AdvancedConfig(
    ...
    public static AdvancedConfig load(Path file) throws IOException {
        ...
        return new AdvancedConfig(
                new Party(bool(toml, "party.enabled", true), integer(toml, "party.invite_timeout_seconds", 60), bool(toml, "party.follow_leader", true), integer(toml, "party.max_size", 20), integer(toml, "party.join_delay_ms", 250), text(toml, "party.command", "party"), text(toml, "party.chat_command", "p"), text(toml, "party.permission", "none")),
    ...
    public static AdvancedConfig defaults() {
        return new AdvancedConfig(new Party(true, 60, true, 20, 250, "party", "p", "none"), new Queue(true, 2, 5, 500, "", "queue", "none"), new Redis(false, "127.0.0.1", 6379, "", "", false, "", "vn", 5, 3000, 10000, 1000, 30000, "", 30, List.of()), new BackendStates(true, List.of("LOBBY", "WAITING", "AVAILABLE"), true), new ServerManagement(true, "velocity.toml", false));
    }
    ...
    public record Party(boolean enabled, int inviteTimeoutSeconds, boolean followLeader, int maxSize, int joinDelayMs, String command, String chatCommand, String permission) {
        public Party {
            inviteTimeoutSeconds = Math.max(10, inviteTimeoutSeconds);
            maxSize = Math.max(2, Math.min(1000, maxSize));
            joinDelayMs = Math.max(0, Math.min(10000, joinDelayMs));
