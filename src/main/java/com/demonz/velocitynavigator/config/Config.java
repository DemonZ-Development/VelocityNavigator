...
public final class Config {
    ...
    private final MenuTokenSettings menuToken;
    ...
    public Config(
        ...
        this(configVersion, commands, routing, healthChecks, messages, updateChecker, metrics, debug,
                circuitBreaker, degradation, geoRouting, notifyOnStartup, notifyAdminsOnJoin,
                startup, lobbyFallback, bedrock, dashboard, language, new StorageConfig("file", "localhost", 3306, "velocitynavigator", "vn", "", "data/velocitynavigator.db", 5, 5000L), AuthConfig.defaults(), new MenuTokenSettings(60));
    }
    ...
    public Config(
        ...
        this.storage = storage == null ? new StorageConfig("file", "localhost", 3306, "velocitynavigator", "vn", "", "data/velocitynavigator.db", 5, 5000L) : storage;
        this.auth = auth == null ? AuthConfig.defaults() : auth;
        this.menuToken = menuToken == null ? new MenuTokenSettings(60) : menuToken;
    }
    ...
    public static Config defaults() {
        return new Config(
                ...
                new BedrockSettings(false, true, true, true, true, "<gradient:#8EF7FF:#D9F7FF><bold>Lobby Selector</bold></gradient>", "<gray>Select a lobby server to connect:</gray>", "<white><bold>{server}</bold></white> <gray>({players} Players)</gray>"),
                DashboardSettings.disabled(),
                LanguageBundle.defaults(),
                new StorageConfig("file", "localhost", 3306, "velocitynavigator", "vn", "", "data/velocitynavigator.db", 5, 5000L),
                AuthConfig.defaults(),
                new MenuTokenSettings(60)
        );
    }
    ...
    public MenuTokenSettings menuToken() {
        return menuToken;
    }

    public record MenuTokenSettings(int sessionSeconds) {
        public MenuTokenSettings {
            sessionSeconds = Math.max(5, Math.min(3600, sessionSeconds));
        }

        public static MenuTokenSettings defaults() {
            return new MenuTokenSettings(60);
        }
    }
    ...
    public record AuthConfig(
            ...
            boolean enable2fa,
            int pinLength,
            boolean voidWorldHolding,
            String holdingServer,
            int sessionTimeoutMinutes,
            int minPasswordLength,
            boolean bedrockFormEnabled
    ) {
        public AuthConfig {
            holdingServer = holdingServer == null ? "" : holdingServer.trim();
            sessionTimeoutMinutes = Math.max(1, sessionTimeoutMinutes);
            minPasswordLength = Math.max(4, minPasswordLength);
        }

        public static AuthConfig defaults() {
            return new AuthConfig(false, "argon2id", false, 6, true, "", 60, 8, true);
        }
    }
    ...
    public record GeoRoutingSettings(
            boolean enabled,
            String databasePath,
            String provider,
            boolean fallbackEnabled,
            String fallbackMode,
            Map<String, List<String>> affinityCountries
    ) {
        public GeoRoutingSettings(boolean enabled, String databasePath) {
            this(enabled, databasePath, "maxmind", true, "ip_api", Map.of());
        }

        public GeoRoutingSettings {
            databasePath = databasePath == null ? "" : databasePath;
            provider = provider == null || provider.isBlank() ? "maxmind" : provider.toLowerCase(Locale.ROOT);
            fallbackMode = fallbackMode == null || fallbackMode.isBlank() ? "ip_api" : fallbackMode.toLowerCase(Locale.ROOT);
            if (affinityCountries == null) affinityCountries = Map.of();
        }
    }
    ...
    public record StorageConfig(
        ...
        public StorageConfig {
            if (type == null || type.isBlank()) type = "file";
            type = type.trim().toLowerCase(Locale.ROOT);
            if (host == null) host = "localhost";
            if (port <= 0) port = 3306;
            if (database == null) database = "velocitynavigator";
            if (username == null) username = "vn";
            if (password == null) password = "";
            if (sqliteFile == null || sqliteFile.isBlank()) sqliteFile = "data/velocitynavigator.db";
            if (poolSize <= 0) poolSize = 5;
            if (connectionTimeoutMs <= 0) connectionTimeoutMs = 5000L;
        }
    }
}
