...
public final class MenuBridgeProtocol {
    ...
    public static final String BACKEND_SELECTION_TOKEN = "backend";
    private static final int MAGIC = 0x564E4D31;
    private static final int OPEN = 1;
    private static final int SELECT = 2;
    private static final int HELLO = 3;
    private static final int PARTY_STATE = 4;
    private static final int AUTH_STATUS = 5;
    ...
    public static byte[] encodePartyState(PartyState state) throws IOException {
        if (state == null) {
            throw new IllegalArgumentException("Party state is required");
        }
        List<String> members = state.members() == null ? List.of() : state.members();
        if (members.size() > 100) {
            throw new IllegalArgumentException("Party state contains too many members");
        }
        ByteArrayOutputStream bytes = new ByteArrayOutputStream();
        try (DataOutputStream out = new DataOutputStream(bytes)) {
            out.writeInt(MAGIC);
            out.writeByte(PARTY_STATE);
            out.writeBoolean(state.inParty());
            writeBounded(out, state.name(), 256);
            writeBounded(out, state.leader(), 64);
            out.writeShort(Math.max(0, Math.min(100, state.size())));
            out.writeShort(Math.max(1, Math.min(100, state.maxSize())));
            out.writeBoolean(state.isLeader());
            writeBounded(out, state.role(), 32);
            out.writeBoolean(state.isOpen());
            out.writeByte(members.size());
            for (String member : members) {
                writeBounded(out, member, 64);
            }
        }
        return bytes.toByteArray();
    }

    public static PartyState decodePartyState(byte[] payload) throws IOException {
        try (DataInputStream in = input(payload)) {
            requireHeader(in, PARTY_STATE);
            boolean inParty = in.readBoolean();
            String name = readBounded(in, 256);
            String leader = readBounded(in, 64);
            int size = in.readUnsignedShort();
            int maxSize = in.readUnsignedShort();
            boolean isLeader = in.readBoolean();
            String role = readBounded(in, 32);
            boolean isOpen = in.readBoolean();
            int memberCount = in.readUnsignedByte();
            if (size > 100 || maxSize < 1 || maxSize > 100 || memberCount > 100) {
                throw new IOException("Invalid party state metadata");
            }
            List<String> members = new ArrayList<>(memberCount);
            for (int index = 0; index < memberCount; index++) {
                members.add(readBounded(in, 64));
            }
            requireFullyRead(in);
            return new PartyState(inParty, name, leader, size, maxSize, isLeader, role, isOpen,
                    List.copyOf(members));
        }
    }

    public static byte[] encodeAuthStatus(boolean authenticated) throws IOException {
        ByteArrayOutputStream bytes = new ByteArrayOutputStream();
        try (DataOutputStream out = new DataOutputStream(bytes)) {
            out.writeInt(MAGIC);
            out.writeByte(AUTH_STATUS);
            out.writeBoolean(authenticated);
        }
        return bytes.toByteArray();
    }

    public static boolean decodeAuthStatus(byte[] payload) throws IOException {
        try (DataInputStream in = input(payload)) {
            requireHeader(in, AUTH_STATUS);
    ...
    public static byte[] encodeAuthRequest(boolean register, String password, String repeatPassword) throws IOException {
        requireNonBlank(password, "password");
        if (password.length() > 64 || (repeatPassword != null && repeatPassword.length() > 64)) {
            throw new IllegalArgumentException("Menu bridge password is outside the allowed length range");
        }
        ByteArrayOutputStream bytes = new ByteArrayOutputStream();
        try (DataOutputStream out = new DataOutputStream(bytes)) {
            out.writeInt(MAGIC);
            out.writeByte(AUTH_REQUEST);
            out.writeBoolean(register);
            writeBounded(out, password, 64);
            writeBounded(out, repeatPassword == null ? "" : repeatPassword, 64);
        }
        return bytes.toByteArray();
    }

    public static AuthRequest decodeAuthRequest(byte[] payload) throws IOException {
        try (DataInputStream in = input(payload)) {
            requireHeader(in, AUTH_REQUEST);
            boolean register = in.readBoolean();
            String password = readBounded(in, 64);
            String repeatPassword = readBounded(in, 64);
            if (password.isBlank()) {
                throw new IOException("Auth request password is required");
            }
            requireFullyRead(in);
            return new AuthRequest(register, password, repeatPassword);
        }
    }
    ...
    public static PacketType packetType(byte[] payload) throws IOException {
        try (DataInputStream in = input(payload)) {
            ...
            return switch (in.readUnsignedByte()) {
                ...
                case PARTY_STATE -> PacketType.PARTY_STATE;
                case AUTH_STATUS -> PacketType.AUTH_STATUS;
                case AUTH_REQUEST -> PacketType.AUTH_REQUEST;
    ...
    public record PartyState(boolean inParty, String name, String leader, int size, int maxSize,
                             boolean isLeader, String role, boolean isOpen, List<String> members) {
    }

    public record AuthRequest(boolean register, String password, String repeatPassword) {
    }
}
